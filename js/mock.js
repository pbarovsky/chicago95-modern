...
if (!('lightdm' in window)) {
    document.title = "chicago95-modern - Live Demo";
    window.lightdm = {};

    lightdm.hostname = "cachyos";

    lightdm.sessions = [
        { key: "bspwm", name: "bspwm", comment: "no comment" },
        { key: "openbox", name: "Openbox", comment: "no comment" },
        { key: "plasma", name: "KDE Plasma", comment: "no comment" }
    ];
    ...
    lightdm.suspend = () => show_message("Suspending...");
    lightdm.hibernate = () => show_message("Hibernating...");
    lightdm.restart = () => show_message("Restarting...");
    lightdm.shutdown = () => show_message("Shutting down...");
    ...
    lightdm.start_session_sync = function (session) {
        ...
        if (!lightdm.is_authenticated) {
            ...
            show_error("Invalid session!");
        } else {
            show_message("Logged in successfully!");
...
const _lightdm_mock_get_user = username =>
    lightdm.users.find(user => user.name === username);
