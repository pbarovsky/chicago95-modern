...
this.addEventListener("load", () => {
    ...
    lightdm.users.forEach(usr => {
        $("user-dropdown").appendChild(
            make_menu_item(
                `<img src="static/stock_person.svg" class="sm-avatar"></img>${usr.display_name}`,
                () => set_user(usr)
            )
        );
    });
    ...
    lightdm.sessions.forEach(s => {
        $("session-list").appendChild(
            make_menu_item(s.name, () => set_session(s))
        );
    });
...
function authentication_complete() {
    ...
    if (lightdm.is_authenticated) {
        ...
        if (currentUser) {
            lightdm.start_authentication(currentUser.name);
        }
...
function set_user(user) {
    ...
    $("avatar").src = "static/stock_person.svg";
